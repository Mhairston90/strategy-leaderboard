...
/**
 * Hermes Decisions Ledger (data/hermes/decisions.md) — the recommendation ->
 * outcome loop. Rows are appended by the human (or a human-approved agent)
 * when a queue item gets a decision; the Outcome column is filled after
 * results are known.
 */
export function parseHermesDecisionsText(text) {
  return parseMarkdownTable(text)
    .map(row => ({
      decidedAt: row['decided (utc)'] || row.decided || '',
      owner: row.owner || '',
      strategy: row.strategy || '',
      titleMatch: row['title match'] || '',
      decision: row.decision || '',
      actionTaken: row['action taken'] || '',
      outcome: row.outcome || '',
    }))
    .filter(row => row.strategy && row.decision);
}

/**
 * Decision Desk — joins live queue items with the item tracker (ages) and
 * the decisions ledger (decision state). Actionable = priority <= 2 or
 * repair/experiment; everything else is sample-collection noise, collapsed
 * by default in the render. Priority-1 items undecided for >= 7 days are
 * flagged ESCALATED.
 */
export function buildDecisionDesk(queueItems, trackerText, decisionsText, nowMs = Date.now()) {
  let tracker = {};
  try { tracker = (JSON.parse(trackerText || '{}').items) || {}; } catch { tracker = {}; }
  const decisions = parseHermesDecisionsText(decisionsText || '');
  const keyOf = it => ['owner', 'strategy', 'type', 'title']
    .map(k => String(it[k] ?? '').trim().toLowerCase()).join('|');
  const findDecision = it => decisions.find(d =>
    d.strategy.toLowerCase() === String(it.strategy || '').toLowerCase()
    && (!d.titleMatch || String(it.title || '').toLowerCase().includes(d.titleMatch.toLowerCase()))
    && (!d.owner || d.owner.toLowerCase() === String(it.owner || '').toLowerCase())) || null;
  const enriched = (queueItems || []).map(it => {
    const rec = tracker[keyOf(it)];
    const first = rec && rec.first_seen ? Date.parse(rec.first_seen) : NaN;
    const ageDays = Number.isFinite(first) ? Math.max(0, Math.floor((nowMs - first) / 86400000)) : null;
    const decision = findDecision(it);
    const priority = Number(it.priority ?? 3);
    const escalated = priority === 1 && !decision && ageDays != null && ageDays >= 7;
    return { ...it, priority, ageDays, decision, escalated };
  });
  const isActionable = it => it.priority <= 2 || it.type === 'repair' || it.type === 'experiment';
  const actionable = enriched.filter(isActionable)
    .sort((a, b) => (a.priority - b.priority) || ((b.ageDays ?? 0) - (a.ageDays ?? 0)));
  const samples = enriched.filter(it => !isActionable(it));
  return {
    actionable,
    samples,
    undecided: actionable.filter(i => !i.decision).length,
    decided: actionable.filter(i => i.decision).length,
    escalated: actionable.filter(i => i.escalated).length,
  };
}
...
export function buildHermesMonitorModel(input = {}) {
  ...
  const decisionDesk = buildDecisionDesk(
    queueItems, input.itemTrackerText || '', input.decisionsText || '',
...
export function renderHermesMonitorHtml(model) {
  ...
  return `
    ...
    ${decisionDeskHtml(model.decisionDesk)}
...
function decisionRowHtml(it) {
  const dec = it.decision
    ? `<span class="status-pill status-ok" title="${escapeHtml(it.decision.actionTaken || '')}">${escapeHtml(it.decision.decision)}${it.decision.outcome ? ' \u2192 ' + escapeHtml(it.decision.outcome) : ''}</span>`
    : it.escalated
      ? '<span class="status-pill status-warn">ESCALATED \u2014 undecided</span>'
      : '<span class="status-pill status-pending">undecided</span>';
  const age = it.ageDays == null ? '' : `<span class="dim">${it.ageDays}d old</span>`;
  return `
    <div class="source-row source-${it.escalated ? 'warn' : 'ok'}">
      <strong>P${escapeHtml(it.priority)} \u00b7 ${escapeHtml(it.owner)}</strong>
      <span title="${escapeHtml(it.action || '')}">${escapeHtml(it.title)}</span>
      ${age}
      ${dec}
    </div>
  `;
...
function decisionDeskHtml(desk) {
  if (!desk) return '';
  const summary = `${desk.actionable.length} actionable \u00b7 ${desk.undecided} undecided \u00b7 ${desk.decided} decided \u00b7 ${desk.escalated} escalated`;
  const samplesBlock = desk.samples.length
    ? `<details class="dd-samples"><summary class="dim">${desk.samples.length} sample-collection items (collapsed \u2014 no decision needed)</summary>
        <div class="queue-source-list">${desk.samples.map(decisionRowHtml).join('')}</div>
      </details>`
    : '';
  return `
    <section class="monitor-panel full-width" aria-label="Decision Desk">
      <div class="panel-head">
        <h2>Decision Desk</h2>
        <span class="status-pill status-${desk.escalated ? 'warn' : desk.undecided ? 'pending' : 'ok'}">${escapeHtml(summary)}</span>
      </div>
      <p class="dim">Items needing a human call, with age from first sighting (data/hermes/item_tracker.json). Record decisions in data/hermes/decisions.md \u2014 fill the Outcome column once results land, closing the recommendation\u2192outcome loop.</p>
      <div class="queue-source-list">
        ${desk.actionable.map(decisionRowHtml).join('') || '<div class="empty-monitor">No actionable items \u2014 queue is all sample collection.</div>'}
      </div>
      ${samplesBlock}
    </section>
  `;
...
function normalizeQueue(queue, owner) {
  const items = Array.isArray(queue?.items)
    ? queue.items.map(item => ({
        ...
        action: String(item?.requested_action || ''),
        owner,
      }))
    : [];
  return {

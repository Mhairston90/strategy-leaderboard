...
test('parsePortfolioOpenPositions reads stocks Symbol layout (no Pair/Mark/PnL columns)', () => {
  // Stocks MR/Trend portfolios use "Symbol" (not "Pair") and carry no live
  // mark or P&L column. They must still SHOW (exposure = size × entry).
  const portfolioStocks = `# Stocks MR v1
...
`;
  const positions = parsePortfolioOpenPositions(portfolioStocks, 'Stocks MR v1');
  assert.equal(positions.length, 2);
  assert.equal(positions[0].pair, 'NVDA');
  assert.equal(positions[0].entry, 208.75);
  assert.equal(positions[0].unrealizedPnl, null);                  // no P&L column → null, not garbage
  assert.ok(Math.abs(positions[0].exposure - 13.1041 * 208.75) < 0.01); // size × entry
});

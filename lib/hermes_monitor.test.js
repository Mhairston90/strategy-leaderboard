...
// ---- Decision Desk: lifecycle + decisions loop (added 2026-06-10) ----
import { buildDecisionDesk, parseHermesDecisionsText } from './hermes_monitor.js';

test('decision desk ages items, joins decisions, escalates stale P1s', () => {
  const items = [
    { owner: 'claude', strategy: 'Stocks Basket Breakout v1', type: 'experiment', priority: 1,
      title: 'Address regime collapse in Stocks Basket Breakout v1', action: 'decide within the week' },
    { owner: 'claude', strategy: 'Stocks Mean Reversion v1', type: 'experiment', priority: 1,
      title: 'Defend Stocks Mean Reversion v1', action: 'halve size or pause' },
    { owner: 'fable', strategy: 'FABLE Crypto Drift v1', type: 'sample_collection', priority: 3,
      title: 'Keep collecting forward sample for FABLE Crypto Drift v1', action: 'wait' },
  ];
  const now = Date.parse('2026-06-20T00:00:00Z');
  const tracker = JSON.stringify({ items: {
    'claude|stocks basket breakout v1|experiment|address regime collapse in stocks basket breakout v1':
      { first_seen: '2026-06-10T00:00:00Z' },
    'claude|stocks mean reversion v1|experiment|defend stocks mean reversion v1':
      { first_seen: '2026-06-18T00:00:00Z' },
  }});
  const decisions = [
    '| Decided (UTC) | Owner | Strategy | Title match | Decision | Action taken | Outcome |',
    '|---|---|---|---|---|---|---|',
    '| 2026-06-19T00:00:00Z | claude | Stocks Mean Reversion v1 | Defend | ack | halved size on new entries | |',
  ].join('\n');

  const desk = buildDecisionDesk(items, tracker, decisions, now);
  assert.equal(desk.actionable.length, 2);
  assert.equal(desk.samples.length, 1);
  const collapse = desk.actionable.find(i => i.title.includes('regime collapse'));
  assert.equal(collapse.ageDays, 10);
  assert.equal(collapse.escalated, true);          // P1, 10d, undecided
  const mr = desk.actionable.find(i => i.title.includes('Defend'));
  assert.equal(mr.escalated, false);               // decided -> never escalates
  assert.equal(mr.decision.decision, 'ack');
  assert.equal(desk.undecided, 1);
  assert.equal(desk.escalated, 1);
});

test('parseHermesDecisionsText reads ledger rows and skips blanks', () => {
  const rows = parseHermesDecisionsText([
    '| Decided (UTC) | Owner | Strategy | Title match | Decision | Action taken | Outcome |',
    '|---|---|---|---|---|---|---|',
    '| 2026-06-19T00:00:00Z | fable | FABLE Equities Fader v1 | Repair weak edge | dismissed | insurance leg, bleed expected | |',
    '|  |  |  |  |  |  |  |',
  ].join('\n'));
  assert.equal(rows.length, 1);

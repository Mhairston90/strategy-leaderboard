...
test('post-repeal: owners are scored top-5 forward with every strategy eligible (no pre-registration)', () => {
  // Pre-registration was REPEALED 2026-06-16 (scoring_registrations.js, by
  // Marcus): the only exclusion is backtested/pre-live-start trades; every
  // forward-trading strategy is eligible regardless of source. So even in June
  // a big forward winner counts — it is no longer barred for being unregistered.
  const big = LOG([
  ...
  assert.equal(fable.registered, false);             // no active registration post-repeal
  assert.equal(fable.sum, 5010);                     // BOTH forward trades count now
  assert.ok(fable.list.some(r => r.name.includes('Fader')));  // monster winner is eligible
});

test('unregistered owner falls back to flagged legacy top-5', () => {
  const m = buildContestScoreboard([
    snap('CODEX Pulse v0', '2026-05-04T00:00:00Z', tlMixed),
  ], new Date('2026-05-15T00:00:00Z').getTime());   // May: no registrations exist
  const codex = m.owners.find(o => o.owner === 'CODEX');

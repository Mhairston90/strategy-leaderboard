 ...
 *
 * Multi-owner since 2026-06-10 (FABLE entry + pre-registration rule):
 * OPUS vs CODEX vs FABLE, scored ONLY on forward P&L (trades entered
 * on/after each strategy's effective contest cutoff).
 *
 * Scoring per owner:
 *   - If the owner PRE-REGISTERED 5 strategies for the current UTC month
 *     (scoring_registrations.js), the score is the summed forward P&L of
 *     exactly those 5. Registered rows that are missing/erroring score 0.
 *   - Otherwise: legacy top-5-by-forward-P&L fallback, flagged
 *     "unregistered" — kept so the board renders while an owner hasn't
 *     picked yet, but visually second-class.
 *
 * Forward P&L comes from the provenance ledger so the scoreboard and the
 * legitimacy audit can never disagree. Exact-twin strategies (identical
 * trade logs) are deduplicated within each owner before scoring.
 */
...
function col(o) {
  const tag = o.registered
    ? '<span class="cs-reg" title="pre-registered 5 for this month">registered</span>'
    : '<span class="cs-unreg" title="no pre-registration this month: legacy top-5 fallback">unregistered</span>';
...
export function renderContestScoreboardHtml(model) {
  ...
  return `
    ...
    <p class="dim cs-note">Forward P&amp;L only (trades entered on/after each strategy's contest cutoff). Registered owners are scored on exactly their pre-registered 5; unregistered owners show a flagged legacy top-5 until they register (scoring_registrations.js).</p>

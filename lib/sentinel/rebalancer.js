...
const NINJA_MONTH_NAMES = {
  JAN: '01', FEB: '02', MAR: '03', APR: '04', MAY: '05', JUN: '06',
  JUL: '07', AUG: '08', SEP: '09', OCT: '10', NOV: '11', DEC: '12',
};

// Canonicalize both NinjaTrader instrument spellings to "ROOT MM-YY":
// OIF/config use "MBT 07-26" while Sim101/Tradovate position feedback uses
// "MBT JUL26". Both refer to the same contract.
export function normalizeNinjaInstrument(value) {
  const text = String(value ?? '').trim().toUpperCase().replace(/\s+/g, ' ');
  const monthName = /^([A-Z0-9]+) ([A-Z]{3})(\d{2})$/.exec(text);
  if (monthName && NINJA_MONTH_NAMES[monthName[2]]) {
    return `${monthName[1]} ${NINJA_MONTH_NAMES[monthName[2]]}-${monthName[3]}`;
  }
  const numeric = /^([A-Z0-9]+) (\d{1,2})-(\d{2})$/.exec(text);
  if (numeric) {
    return `${numeric[1]} ${numeric[2].padStart(2, '0')}-${numeric[3]}`;
  }
  return text;
}
...
function ninjaPositionByInstrument(positions, instrument) {
  const normalized = normalizeNinjaInstrument(instrument);
  return (
    asArray(positions).find((position) => normalizeNinjaInstrument(position?.instrument) === normalized) ?? null
  );
}

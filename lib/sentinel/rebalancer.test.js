...
test('buildRebalanceOrders matches Sim101 month-name position instruments to configured contracts', () => {
  // Sim101/Tradovate position feedback names instruments "MBT JUL26" while the
  // sentinel config and OIF orders use "MBT 07-26" — they are the same contract.
  const result = buildRebalanceOrders({
    routedTargets: [
      {
        symbol: 'BTC/USD',
        ...
        point_value: 0.1,
        account: 'Sim101',
        target_notional_usd: 18570,
        reference_price_usd: 61900,
      },
    ],
    ninjaPositions: [
      {
        account: 'Sim101',
        instrument: 'MBT JUL26',
        exchange: 'Globex',
    ...
    now: '2026-07-03T07:25:00Z',
  });

  assert.deepEqual(result.orders, [], 'an existing month-name position should satisfy the target');
});

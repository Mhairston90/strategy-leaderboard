...
function addAnomaly(anomalies, event, reasons) {
  anomalies.push({
    ...
    reason: reasons.join('; '),
    event: event && typeof event === 'object' ? { ...event } : event,
  });
}
...
function applyFill(orders, positions, anomalies, event) {
  ...
  const previousOrder = brokerOrderId ? orders.get(brokerOrderId) : null;
  ...
  const strategy = eventStrategy(event) ?? eventStrategy(previousOrder);
  const sameStrategy =
    !existing.strategy || !strategy || existing.strategy === strategy;
  ...
  positions.set(symbol, {
    ...
    ...(sameStrategy && (existing.strategy || strategy)
      ? { strategy: existing.strategy ?? strategy }
      : {}),
  });
}
...
export function replayLedgerEvents(events = []) {
  ...
  for (const rawEvent of Array.isArray(events) ? events : []) {
    ...
    if (!event) {
      addAnomaly(anomalies, rawEvent, ['event must be an object']);
      continue;
    }
    ...
    if (event.type === 'order_submitted') {
      ...
      addAnomaly(anomalies, event, ['unknown event type']);
    }
  }

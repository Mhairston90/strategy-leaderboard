...
test('replayLedgerEvents records unknown and malformed event anomalies without throwing', () => {
  const state = replayLedgerEvents([
  ...
  assert.equal(state.anomalies.length, 4);
  assert.deepEqual(
    state.anomalies.map((anomaly) => anomaly.event_type),
    ['unknown_event', '', '', 'order_filled'],
  );
});
...
test('replayLedgerEvents preserves strategy from submitted order on filled positions', () => {
  const state = replayLedgerEvents([
    {
  ...
  assert.equal(state.orders.get('o1').strategy, 'CODEX Aggro v0');
  assert.equal(state.positions.get('AAPL').strategy, 'CODEX Aggro v0');
});

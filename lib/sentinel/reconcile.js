...
function submittedOrderSymbols(submittedOrders) {
  ...
  for (const order of submittedOrders.values()) {
    ...
    if (order?.status === 'submitted' && symbol) {
      symbols.add(symbol);
    }
  }
...
function isExpectedSubmittedPosition(position, expectedSubmittedSymbols) {
  return (
    position?.qty !== null &&
    position?.market_value !== null &&
    expectedSubmittedSymbols.has(position.symbol)
  );
}
...
export function compareLedgerToBroker({ ledgerPositions, brokerPositions, submittedOrders } = {}) {
  ...
  const expectedSubmittedSymbols = submittedOrderSymbols(submittedOrders);
  ...
  for (const symbol of symbols) {
    ...
    if (!ledgerPosition) {
      if (
        ...
        !isExpectedSubmittedPosition(brokerPosition, expectedSubmittedSymbols)
      ) {

...
test('ticketToAlpacaOrder maps normalized ticket to Alpaca body', () => {
  assert.deepEqual(ticketToAlpacaOrder(baseTicket), {
    ...
    time_in_force: 'day',
    notional: '25.00',
    client_order_id: 'sentinel-20260628-000001-signal-1',
  });
});
...
test('ticketToAlpacaOrder derives sanitized deterministic client_order_id from ticket id and source signal', () => {
  const order = ticketToAlpacaOrder({
    ...baseTicket,
    ticket_id: 'Sentinel / Ticket: 1234567890123456789012345678901234567890',
    source_signal_id: 'Signal With Spaces + Symbols! 9876543210987654321098765432109876543210',
  });

  assert.match(order.client_order_id, /^[A-Za-z0-9_-]+$/);
  assert.equal(order.client_order_id.length <= 48, true);
  assert.equal(

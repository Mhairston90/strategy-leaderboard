...
function recentOrdersForSymbol(ticket, recentTickets) {
  ...
  return asArray(recentTickets).filter((recentTicket) => {
    ...
    return recentTime !== null && recentTime <= ticketTime && ticketTime - recentTime < oneHourMs;
  }).length;
}
...
export function evaluateTicketRisk(ticket, context = {}) {
  ...
  if (maxOrdersPerSymbolPerHour !== null) {
    const recentSymbolOrders = recentOrdersForSymbol(ticket, context?.recentTickets);

...
test('buildReconciliationUpdate freezes oversized broker-only position despite same-symbol submitted order', () => {
  const result = buildReconciliationUpdate({
    ledgerEvents: [
      {
        type: 'order_submitted',
    ...
    brokerPositions: [{ symbol: 'AAPL', qty: '1000', market_value: '250000' }],
  });

  assert.equal(result.report.status, 'error');
  assert.match(result.report.freeze_reason, /position mismatch/);
  assert.equal(result.report.differences[0].type, 'broker_only');
  assert.equal(result.riskState.frozen, true);
});

test('buildReconciliationUpdate freezes wrong-direction broker-only position despite submitted order', () => {
  const result = buildReconciliationUpdate({
    ledgerEvents: [
      {
        type: 'order_submitted',
    ...
    brokerPositions: [{ symbol: 'AAPL', qty: '-1', market_value: '-99' }],
  });
...
test('buildReconciliationUpdate freezes on unknown or malformed ledger events', () => {
  const result = buildReconciliationUpdate({
    ledgerEvents: [null, { type: 'unknown_event', symbol: 'AAPL' }],
    brokerPositions: [],
  });

  assert.equal(result.report.status, 'error');
  assert.match(result.report.freeze_reason, /ledger anomal/);
  assert.equal(result.report.ledger_anomalies.length, 2);
  assert.equal(result.riskState.frozen, true);
});

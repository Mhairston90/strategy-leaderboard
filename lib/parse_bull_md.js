...
export function parseTradeLog(md) {
  ...
  for (const rawLine of md.split('\n')) {
    ...
    // Variant-schema rows (| ... | R | Reason | Variant |, e.g. v0.14/v0.15)
    // carry NO dedicated $ PnL column — the dollar amount lives in the Reason
    // prose ("... = +$319.57/+1.97R"). In that layout the cell the destructure
    // reads as `pnl` is actually the prose. Detect a CLOSE with a numeric R but
    // a non-numeric pnl cell, and pull the $ out of the prose so the trade is
    // counted (otherwise these strategies show 0 trades on the board).
    if (action === 'CLOSE' && pnlVal == null && rVal != null) {
      const prose = String(pnl);
      const m = prose.match(/=\s*([+\-−]?)\s*\$\s*([\d,]+(?:\.\d+)?)/);
      if (m) {
        pnlVal = (m[1] === '-' || m[1] === '−' ? -1 : 1) * parseFloat(m[2].replace(/,/g, ''));
        reasonText = prose; // the prose is more informative than the variant tag
      }
    }

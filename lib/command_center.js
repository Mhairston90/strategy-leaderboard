...
import { buildTripsWithEntryTime, parseTradeLog } from './parse_bull_md.js';
...
export function parsePortfolioOpenPositions(text, strategyName = '') {
  ...
  const positions = [];
  for (const cells of markdownRows(text)) {
    if (cells.length < 9 || cells[0] === 'Pair') continue;
    const [pair, sleeve, side, size, entry, stop, mark, pnl, exposure] = cells;
    if (!pair || pair === 'No open positions.') continue;
    const exposureValue = Math.abs(moneyOrNull(exposure) ?? 0);
    ...
    positions.push({
      ...
      entry: numberOrNull(entry),
      stop: numberOrNull(stop),
      mark: numberOrNull(mark),
      unrealizedPnl: moneyOrNull(pnl),

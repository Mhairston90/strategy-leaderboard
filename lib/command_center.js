...
// not by fixed position, so this reads every camp's portfolio layout:
//   CODEX  — Pair|Sleeve|Side|Size|Entry|Stop|MTM price|Unrealized PnL|Exposure
//   BULL   — Pair|Side|Size|Entry|Stop (initial)|Active stop|Target|Entry ts|
//            Last (MTM)|Unrealized R|Unrealized $
//   FABLE  — Pair|Sleeve|Side|Size|Entry|Stop|Mark|PnL|Exposure|Entry time|...
//   Stocks — Symbol|Entry|Stop|Size|Entry time|Bars held  (no Side/Mark/PnL;
//            exposure falls back to size × entry so the row still shows)
// The old positional destructuring only fit the CODEX layout; on BULL rows the
// entry timestamp landed in the P&L column and rendered the year ("2026") as
// "$2026.00" with a bogus % (the 2026-06-20 SOL display bug), and stocks rows
// (Symbol header) were skipped entirely.
export function parsePortfolioOpenPositions(text, strategyName = '') {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if ((norm.includes('pair') || norm.includes('symbol')) && norm.includes('size')) {
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    const entry = numberOrNull(at(cells, iEntry));
    const mark = numberOrNull(at(cells, iMark));
    // Exposure: prefer an explicit column; else notional from size × current
    // mark, or size × entry when the snapshot carries no live mark (stocks
    // MR/Trend portfolios log Symbol|Entry|Stop|Size only — no Mark/PnL).
    let exposure = Math.abs(moneyOrNull(at(cells, iExp)) ?? 0);
    if (exposure <= 0 && size != null) {
      const basis = mark ?? entry;
      if (basis != null) exposure = Math.abs(size * basis);
    }
...
function openTradeMonitorHtml(openTrades) {
  const rows = openTrades.map(trade => `
    <div class="trade-table-row open-trade-row">
      ...
      <span class="${escapeHtml(trade.unrealizedPnl == null ? 'dim' : (trade.unrealizedPnl >= 0 ? 'pos' : 'neg'))}" data-label="Live P/L"${trade.unrealizedPnl == null ? ' title="no live price in this strategy\'s feed — P&L unknown"' : ''}>
        ${escapeHtml(trade.unrealizedPnl == null ? 'no mark' : fmtMoneyFine(trade.unrealizedPnl))}

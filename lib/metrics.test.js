...
test('maxDrawdown with startingCapital: realistic small-pnl-on-large-account', () => {
  // $10k account, trade pnls like BULL's. DD% should be small, not 6000%+.
  const pnls = [-9.14, -26.69, 39.40, -38.77, -34.04, -34.68, -28.77, -33.82, -56.38];
  const dd = maxDrawdown(pnls, 10000);
  assert.ok(dd > -5, `expected dd > -5%, got ${dd}`);
  assert.ok(dd < 0, `expected negative dd, got ${dd}`);
});

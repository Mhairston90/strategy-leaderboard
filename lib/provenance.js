...
export function computeProvenance(snapshots) {
  return (snapshots || []).map(s => {
    ...
    if (s.strategy.source.type === 'sheets') {
      // Sheet adapters now attach a provenance summary (entry-time paired).
      const p = s.row?.provenance;
      if (p) {
        return {
          name, isCodex, cutoff, source: p.source || 'sheet-entry-time',
          total: p.total, forward: p.forward, backtest: p.backtest,
          fwdPnl: p.fwdPnl, btPnl: p.btPnl, legitimacy: p.legitimacy,
        };
      }
      // Fallback only if an adapter didn't supply provenance.
      return {
